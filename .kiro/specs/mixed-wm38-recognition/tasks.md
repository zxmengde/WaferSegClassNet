...
- [x] 2. 配置系统实现
  ...
  - [x] 2.1 创建 src/config_schema.py 定义配置数据类

    - ExperimentConfig, DataConfig, ModelConfig, TrainingConfig, LossConfig
    - YAML加载与验证函数
    - _Requirements: 2.7_

  - [x] 2.2 编写属性测试：配置YAML格式有效性

    - **Property 9: 配置文件YAML格式有效性**
    - **Validates: Requirements 2.7**
  - [x] 2.3 创建 configs/e0_template.yaml 基线配置模板
...
- [x] 3. 数据模块实现
  ...
  - [x] 3.1 创建 src/data/mappings.py 标签映射

    - LABEL_38_TO_8 字典（权威来源）
    - map_38_to_8() 和 validate_mapping() 函数
    - 打印覆盖率统计
    - _Requirements: 2.2, 2.3_
  - [x] 3.2 编写属性测试：38→8标签映射正确性


    - **Property 1: 38类到8类标签映射正确性**
    - **Validates: Requirements 2.2, 2.3**
  - [x] 3.3 创建 src/data/dataset.py MixedWM38Dataset类
  ...
  - [x] 3.4 编写属性测试：分割mask二值性


    - **Property 2: 分割mask二值性**
    - **Validates: Requirements 2.4**

  - [x] 3.5 创建 src/data/augmentation.py 数据增强

    - WaferFriendlyAugmentation类
    - 白名单/黑名单验证
    - _Requirements: 2.6_
  - [x] 3.6 编写轻量pytest：数据增强白名单检查


    - 验证增强配置不包含黑名单操作
    - _Requirements: 2.6_

  - [x] 3.7 创建 src/data/dataloader.py 数据加载器
...
- [x] 5. 模型架构实现
  ...
  - [x] 5.1 创建 src/models/encoder.py 编码器
  ...
  - [x] 5.2 编写轻量pytest：权重加载统计字段完整性


    - 验证输出包含matched/missing/unexpected字段
    - _Requirements: 4.2, 4.3_
  - [x] 5.3 创建 src/models/decoder.py 分割解码器


    - 基于现有transposeConvBlock改造
    - _Requirements: 3.1_
  - [x] 5.4 创建 src/models/heads.py 任务头


    - ClassificationHead（38类/8类）
    - SeparationHead（8通道，E3用）
    - _Requirements: 6.1_
  - [x] 5.5 编写轻量pytest：分离头输出形状


    - 验证输出为8通道
    - _Requirements: 6.1_
  - [x] 5.6 创建 src/models/multitask.py 多任务模型
...
- [x] 8. 训练模块实现
  ...
  - [x] 8.1 创建 src/training/trainer.py Trainer类
  ...
  - [x] 8.2 编写轻量pytest：检查点保存/加载


    - 验证模型权重和优化器状态恢复
    - _Requirements: 3.4_
  - [x] 8.3 创建 src/training/utils.py 训练工具
...
- [x] 9. 评估模块实现
  ...
  - [x] 9.1 创建 src/evaluation/metrics.py 指标计算
    - compute_macro_f1(), compute_map()
    - compute_dice(), compute_iou()

    - _Requirements: 3.5_
  - [x] 9.2 编写轻量pytest：指标计算正确性

    - 使用已知输入验证输出
    - _Requirements: 3.5_

  - [x] 9.3 创建 src/evaluation/evaluator.py Evaluator类
...
- [x] 10. 可视化模块实现
  ...
  - [x] 10.1 创建 src/visualization/plots.py
  ...
  - [x] 10.2 创建 src/visualization/overlays.py（伪mask overlay唯一责任模块）
...
- [x] 14. E0基线训练与评估
  ...
  - [x] 14.1 创建 configs/e0.yaml 完整配置
...
- [x] 15. SSL预训练实现
  ...
  - [x] 15.1 创建 src/training/ssl_trainer.py SimCLR训练器


    - 对比学习损失（InfoNCE）
    - 支持MixedWM38 train图像作为无标签数据源（保守fallback）
    - _Requirements: 4.1_
  - [x] 15.2 创建 train_ssl.py SSL预训练入口


    - 支持 --config configs/ssl.yaml
    - _Requirements: 4.1_
  - [x] 15.3 创建 configs/ssl.yaml 和 configs/ssl_debug.yaml
...
- [x] 17. E2长尾模块
  ...
  - [x] 17.1 创建 src/data/sampler.py 加权采样器
    - WeightedClassSampler类
    - _Requirements: 5.1_
  - [x] 17.2 更新损失函数支持Focal/ClassBalanced
    - _Requirements: 5.2_
  - [x] 17.3 创建 configs/e2.yaml 配置
    - sampler: inverse
...
- [x] 18. E3分离模块
  ...
  - [x] 18.1 创建 src/models/separation.py Prototype分离
  ...
  - [x] 18.2 创建 configs/e3.yaml 配置
    ...
    - 基于E1的checkpoint
    - _Requirements: 6.1_
  - [x] 18.3 运行E3评估生成separation_maps
...
- [x] 19. 对比表生成
  ...
  - [x] 19.1 创建 scripts/generate_comparison.py
...
- [x] 20. 报告与PPT生成
  ...
  - [x] 20.1 创建 scripts/generate_report.py
  ...
  - [x] 20.2 创建 scripts/generate_slides_md.py


    - 生成slides/SLIDES.md（10-12页结构）
    - _Requirements: 10.3_
  - [x] 20.3 创建 scripts/build_pptx.py
...
- [x] 21. 教学文档编写
  ...
  - [x] 21.1 创建 docs/LEARNER_GUIDE.md
